...
Abstract:
A key recovery method that uses a known and a partially known secret that forces the recovery to use a brute force mechanism to get to the full secret of the partial secret and that validates the results with the existence of the key on a blockchain, ledger, database... .
...
Once this key has interacted with a blockchain, ledger... it is possible to recover it by creating all possible keys with the known and the partial known secret(s) by guessing the missing part(s) of the partially secret(s).
...
A key generator uses the password and the location's geo coordinates as salt and digests everything with a high computational cost (in our PoC BCrypt) to create a ECDSA key-pair. The precision of the geo location needs to be known down to a meter level but the key generator will add a random and unknown decimal degree up to millimeter depending on the targeted security of the key.
...
Emotions are the best incorporeal storage on meatspace!
...
In our PoC we use BCrypt but any hashing function that introduces a time/cost variable will work, for
example SCrypt or Argon2. We chose BCrypt because of its long successful lifetime and the ability to sequence the hashing for parallelization.  
...
Thanks to my family which is very supportive with my ideas.
